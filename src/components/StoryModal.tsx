        ...
        <div className="overflow-y-auto flex-1">
          ...
          <div className="p-6 md:p-8 space-y-5">
            ...
            <div>
              ...
              <h2 className="font-serif text-xl md:text-2xl text-cream leading-tight">
                {story.title}
              </h2>
            </div>
            ...
            {/* Short Hook */}
            {shortDesc && (
              <p className="font-serif text-base text-cream-muted/80 italic leading-relaxed border-l-2 border-ember/40 pl-4">
                {shortDesc}
              </p>
            )}
            ...
            {story.themes && story.themes.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {story.themes.map((theme) => (
                  <span
                    key={theme}
                    className="text-xs text-cream-muted/70 border border-border/50 px-2.5 py-1 rounded-full bg-charcoal-light/30"
                  >
                    {theme}
                  </span>
                ))}
            ...
            {(story.intensity || story.contentWarnings) && (
              <div className="flex flex-wrap items-center gap-3 text-xs pt-2 border-t border-border/20">
                {story.intensity && (
                ...
                {story.contentWarnings && story.contentWarnings.length > 0 && story.contentWarnings[0] !== "None" && (
                  <span className="text-cream-muted/50">
                    CW: {story.contentWarnings.slice(0, 2).join(" · ")}
                  </span>
                )}
              </div>
            )}
            ...
            {readUrl && (
              <div className="pt-2">
                <a
                  ...
                  className="inline-flex items-center justify-center gap-3 w-full px-6 py-3 bg-ember/15 border border-ember/40 rounded-lg text-ember hover:bg-ember/25 hover:border-ember/60 hover:shadow-[0_0_30px_rgba(210,105,30,0.2)] transition-all duration-300 group"
                >
